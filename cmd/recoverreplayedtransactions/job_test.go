...
func TestRunJobAerospike(t *testing.T) {
	...
	for _, absent := range []bool{false, true} {
		t.Run(fmt.Sprintf("absent=%v", absent), func(t *testing.T) {
			...
			require.NoError(t, e)
			e = recoverySpendAndCreate(ctx, store, child, 1)
			require.NoError(t, e)
			e = recoverySpendAndCreate(ctx, store, grand, 2)
			require.NoError(t, e)
			...
			require.NoError(t, e)
			if !absent {
				e = recoverySpendAndCreate(ctx, store, child, 2)
				require.NoError(t, e)
			}
			e = recoverySpendAndCreate(ctx, store, legitimate, 2)
			require.NoError(t, e)
