...
func TestLegacyHistoricalTestnetSync(t *testing.T) {
	...
	for i, block := range blocks {
		...
		require.NoError(t, err)
		_, _, err = utxos.SpendAndCreate(ctx, accepted.CoinbaseTx, height, utxo.WithCreateOnly(), utxo.WithMinedBlockInfo(utxo.MinedBlockInfo{BlockID: meta.ID, BlockHeight: height}))
		require.NoError(t, err)
