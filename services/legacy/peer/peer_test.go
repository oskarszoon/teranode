...
// later attempt to re-announce it. RequeueInventory must bypass that filter on
// both the trickle and the immediate-send paths.
func TestPeerRequeueInventory(t *testing.T) {
	tests := []struct {
		name            string
		trickleInterval time.Duration
	}{
		{"trickle queue", 10 * time.Millisecond},
		{"immediate send", 0},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			inCfg := &peer.Config{
				...
				ChainParams:            &chaincfg.MainNetParams,
				Services:               0,
				TrickleInterval:        tc.trickleInterval,
				TstAllowSelfConnection: true,
			}
			outCfg := *inCfg
