...
func TestSyncCoordinator_HandleCatchupFailureForPeer(t *testing.T) {
	t.Run("retains progressing current peer on unrelated failure", func(t *testing.T) {
		sc, reg := newTestSyncCoordinator(t)
		reg.Register(&blockchain.PeerInfo{ID: "current"})
		sc.mu.Lock()
...
func TestSyncCoordinator_CatchupFailureReevaluatesCurrentPeer(t *testing.T) {
	for _, failedPeer := range []string{"", "alternative"} {
		for _, tc := range []struct {
			name string
			age  time.Duration
			want string
		}{
			{"recent progress retains current peer", time.Minute, "current"},
			{"higher work preempts stalled peer", 3 * time.Minute, "better"},
			{"no progress evicts expired peer", 6 * time.Minute, "better"},
		} {
			t.Run(failedPeer+"/"+tc.name, func(t *testing.T) {
				sc, reg := newPreemptionTestCoordinator(t, 0, []byte{0x03}, []byte{0x05}, tc.age)
				before, ok := reg.Get("current")
				require.True(t, ok)
				score := before.ReputationScore
				sc.HandleCatchupFailureForPeer(failedPeer, "subtree attempts exhausted")
				require.Equal(t, tc.want, sc.GetCurrentSyncPeer())
				after, ok := reg.Get("current")
				require.True(t, ok)
				require.Equal(t, score, after.ReputationScore, "an unattributed notification must not invent a peer failure")
			})
		}
	}
}
