...
func TestAvailableEventsForState(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			// RUN and CATCHUPBLOCKS. An idled node keeps a resume path that stays
			// under the checkpoint gate; block announcements are stopped from using
			// it in processCatchupChItem, not here.
			expect: []string{
