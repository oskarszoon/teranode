...
func (s *Client) CheckSubtreeFromBlock(ctx context.Context, subtreeHash chainhash.Hash, baseURL string, blockHeight uint32, blockHash, previousBlockHash *chainhash.Hash, inBlockParentHashes []chainhash.Hash) error {
	var parentHashBytes [][]byte
	if len(inBlockParentHashes) > 0 {
		parentHashBytes = make([][]byte, len(inBlockParentHashes))
		for i := range inBlockParentHashes {
			parentHashBytes[i] = inBlockParentHashes[i][:]
		}
	}
