...
func runReplayRecoveryAerospikeAssemblyEndToEnd(t *testing.T, paginated bool) {
	...
	require.NoError(t, err)
	chainStore, err := blockchainsql.New(logger, chainURL, settings)
	require.NoError(t, err)
	chain, err := blockchain.NewLocalClient(logger, settings, chainStore, nil, nil)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, chainStore.Close()) })
	archive := memory.New()
	...
	err = recoverySpendAndCreate(ctx, store, child, 1)
	require.NoError(t, err)
	...
	err = recoverySpendAndCreate(ctx, store, grandchild, 2)
	require.NoError(t, err)
	...
	err = recoverySpendAndCreate(ctx, store, child, 2)
	require.NoError(t, err)
	...
	err = recoverySpendAndCreate(ctx, store, legitimate, 2)
	require.NoError(t, err)
	...
	err = recoverySpendAndCreate(ctx, store, child, 3)
...
// The release store exposes spending and creation as separate operations.
func recoverySpendAndCreate(ctx context.Context, store *astore.Store, tx *bt.Tx, height uint32) error {
	if _, err := store.Spend(ctx, tx, height); err != nil {
		return err
	}
