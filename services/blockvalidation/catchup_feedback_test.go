...
func TestCatchupFeedback_SubtreeStoreCancellationIsNotOutage(t *testing.T) {
	for _, operation := range []string{"exists", "get", "set"} {
		t.Run(operation, func(t *testing.T) {
			...
			_, err = server.fetchAndStoreSubtree(ctx, block, hash, "peer", "http://peer", false, nil)
			require.Error(t, err)
			require.True(t, errors.IsLocalError(err))
...
func TestCatchupFeedback_DiscoveryFailurePreservesPrimaryEvidence(t *testing.T) {
	...
	_, err := server.fetchAndStoreSubtreeAndSubtreeData(context.Background(), context.Background(), block, hash, "primary", "http://primary", snapshot, nil)
	require.Error(t, err)
