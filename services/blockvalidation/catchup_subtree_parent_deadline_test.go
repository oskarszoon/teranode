...
func TestCatchupSubtreeData_DeadlineAttribution(t *testing.T) {
		...
		for _, owner := range []string{"caller", "download"} {
			t.Run(stage+"/"+owner, func(t *testing.T) {
				synctest.Test(t, func(t *testing.T) {
					...
					server.settings.BlockValidation.SubtreeDataFetchTimeout = downloadTimeout
					subtree, err := subtreepkg.NewIncompleteTreeByLeafCount(1)
					require.NoError(t, err)
					require.NoError(t, subtree.AddCoinbaseNode())
					hash := subtree.RootHash()
					data, err := subtree.Serialize()
					require.NoError(t, err)
					require.NoError(t, server.subtreeStore.Set(context.Background(), hash[:], fileformat.FileTypeSubtreeToCheck, data))
