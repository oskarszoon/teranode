...
type CatchupTransport interface {
	...
	FetchHeaders(ctx context.Context, baseURL string, targetHash *chainhash.Hash, locatorHashes []*chainhash.Hash, maxHeaders int) ([]*model.BlockHeader, error)

	// FetchBlock fetches a single block by hash from the peer.
	FetchBlock(ctx context.Context, baseURL string, hash *chainhash.Hash) (*model.Block, error)

	// FetchBlocks fetches a batch of n blocks starting at hash from the peer.
	FetchBlocks(ctx context.Context, baseURL string, hash *chainhash.Hash, n uint32) ([]*model.Block, error)

	// FetchSubtree fetches the raw subtree bytes for the given hash.
	// The caller is responsible for parsing the returned bytes.
	FetchSubtree(ctx context.Context, baseURL string, hash *chainhash.Hash) ([]byte, error)

	// FetchSubtreeData fetches the raw subtree data stream for the given hash.
	// The caller must close the returned ReadCloser.
	FetchSubtreeData(ctx context.Context, baseURL string, hash *chainhash.Hash) (io.ReadCloser, error)
}
