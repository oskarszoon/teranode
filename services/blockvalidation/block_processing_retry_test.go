...
func TestProcessCatchupChItem(t *testing.T) {
	...
	t.Run("external error reports only deduplicated peers actually attempted", func(t *testing.T) {
		peerFailure := &testFailedPeerIDsError{
			err:     errors.NewServiceError("alternative subtree peer failed"),
			peerIDs: []string{"alt-1", "alt-2", "alt-1", ""},
		}
		external := errors.NewExternalError("parallel subtree peers failed")
		external.SetWrappedErr(peerFailure)
		chain := errors.NewProcessingError("worker failed",
			errors.NewServiceError("subtree fetch failed", external))
		require.True(t, errors.Is(chain, errors.ErrExternal))
		var discovered interface{ FailedPeerIDs() []string }
		require.True(t, errors.As(chain, &discovered), "failed-peer carrier must survive real Teranode wrappers")
		...
		require.ElementsMatch(t, []string{"alt-1", "alt-2"}, p2pClient.recordedFailures())
		mockBC := u.blockchainClient.(*blockchain.Mock)
		mockBC.AssertNumberOfCalls(t, "ReportPeerFailure", 2)
		mockBC.AssertCalled(t, "ReportPeerFailure", mock.Anything, mock.Anything, "alt-1", "catchup", mock.Anything)
		mockBC.AssertCalled(t, "ReportPeerFailure", mock.Anything, mock.Anything, "alt-2", "catchup", mock.Anything)
		mockBC.AssertNotCalled(t, "ReportPeerFailure", mock.Anything, mock.Anything, "origin", "catchup", mock.Anything)
	})
	...
	t.Run("external error with empty carrier does not blame uncontacted origin", func(t *testing.T) {
		external := errors.NewExternalError("no peer was contacted")
		external.SetWrappedErr(&testFailedPeerIDsError{err: errors.NewServiceError("selection failed")})
		u, _ := newServer(3, external)
		p2pClient := &catchupPeersP2PMock{}
		u.p2pClient = p2pClient
		b := testBlock()
