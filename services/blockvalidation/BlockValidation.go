	...
	return u.runOncePerBlock(blockHash, opts, func(opts *ValidateBlockOptions) error {
		...
		if useOptimisticMining {
			...
			if ok, err := block.Valid(ctx, u.logger, u.subtreeStore, u.utxoStore, oldBlockIDsMap, blockHeaders, blockHeaderIDs, u.settings, metaRegenerator); !ok {
				...
				//   - sync (CATCHINGBLOCKS/LEGACYSYNCING): a #1031 catchup-ordering
...
//
// It returns true only when the FSM is NEITHER CATCHINGBLOCKS NOR LEGACYSYNCING.
// This is the distinguishing signal between a genuine floater (a not-in-block
// parent with empty/absent BlockIDs that surfaces as ErrBlockIncomplete in
// RUNNING — provably permanent via the SetMinedMulti->MinedSet invariant) and a
// legitimate #1031 catchup-ordering parent (transient during sync, must be
// retried, never persisted invalid).
//
...
func (u *BlockValidation) isCaughtUp(ctx context.Context) bool {
	...
	return *st != blockchain.FSMStateCATCHINGBLOCKS && *st != blockchain.FSMStateLEGACYSYNCING
}
