...
| InitializeNodeInState | string | "" (IDLE for `operator`/`docker.m`) | blockchain_initializeNodeInState | FSM state a fresh node boots into: IDLE, RUNNING or CATCHINGBLOCKS. Empty means CATCHINGBLOCKS. Ignored when a state is already persisted; invalid values fail startup |
