...
func TestRecoveryConditionalMarkersAndReplay(t *testing.T) {
	...
	err = recoverySpendAndCreate(ctx, store, child, 1000)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = recoverySpendAndCreate(ctx, store, child, 1001)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Nil(t, rec)
	err = recoverySpendAndCreate(ctx, store, child, 1002)
...
func recoverySpendAndCreate(ctx context.Context, store *recovery.Store, tx *bt.Tx, height uint32) error {
